...
var script_newHrmEmployees = {
    ...
    validate_CardNo: function(sender) {
      ...
      this.validatePK_newHrmEmployees(sender,Prefix);
      },
    ...
    validatePK_newHrmEmployees: function(o,Prefix) {
      var value = o.id;
      try{$get(Prefix.replace('_F_','') + '_L_ErrMsgnewHrmEmployees').innerHTML = '';}catch(ex){}
      ...
      o.style.backgroundRepeat= 'no-repeat';
      o.style.backgroundPosition = 'right';
      PageMethods.validatePK_newHrmEmployees(value, this.validatedPK_newHrmEmployees);
    },
    validatedPK_newHrmEmployees: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      o.style.backgroundImage  = 'none';
      if(p[0]=='1'){
        try{$get('cph1_FVnewHrmEmployees_L_ErrMsgnewHrmEmployees').innerHTML = p[2];}catch(ex){}
        o.value='';
        o.focus();
      }
    },
