...
Namespace SIS.ATN
  Partial Public Class atnSiteAttendPosting
    Public Shadows ReadOnly Property UnPostVisible() As Boolean
      Get
        Dim mRet As Boolean = False
        Try
          Select Case ATNStatusID
            Case atnAplStates.Posted
              mRet = True
          End Select
    ...
    Public Shared Function UpdateAppliedLeaveStatus(ByVal Context As String) As String
      Try
        ...
        For Each apl As SIS.ATN.LeaveContextDetail In oCon.LeaveContextDetails
          ...
          If oPP.Posted Then Continue For
          oPP.Applied = True
    ...
    Public Shared Function UnPostSiteApplication(ByVal sAtn As SIS.ATN.atnSiteAttendance) As SIS.ATN.atnSiteAttendance
      Try
        Dim oAppl As SIS.ATN.atnApplHeader = SIS.ATN.atnApplHeader.GetByID(sAtn.LeaveApplID)

        Dim oPPs As List(Of SIS.ATN.atnNewAttendance) = SIS.ATN.atnNewAttendance.GetAttendanceByApplHeaderID(oAppl.LeaveApplID)

        For Each oPP As SIS.ATN.atnNewAttendance In oPPs
          Select Case oPP.PunchStatusID
            Case "AF"
              oPP.Applied1LeaveTypeID = ""
              oPP.Posted1LeaveTypeID = ""
              oPP.FinalValue = 0.5
              oPP.NeedsRegularization = True
              oPP.ApplHeaderID = ""
              oPP.Posted = False
              oPP.Applied = False
              oPP.AppliedValue = 0
            Case "AS", "TS"
              oPP.Applied2LeaveTypeID = ""
              oPP.Posted2LeaveTypeID = ""
              oPP.FinalValue = 0.5
              oPP.NeedsRegularization = True
              oPP.ApplHeaderID = ""
              oPP.Posted = False
              ...
              oPP.AppliedValue = 0
            Case "PR", "WO", "HD"
              If oPP.Applied = True AndAlso oPP.ApplStatusID = "" Then
                oPP.PunchStatusID = "AD"
                oPP.Punch1Time = "00.00"
                oPP.Punch2Time = "00.00"
                oPP.NeedsRegularization = True
                oPP.PunchValue = 0
                oPP.FinalValue = 0
                oPP.ApplHeaderID = ""
          ...
          SIS.ATN.atnNewAttendance.Update(oPP)
        Next
        SIS.ATN.atnLeaveLedger.DeleteByApplHeaderID(oAppl.LeaveApplID)
        SIS.ATN.atnApplHeader.Delete(oAppl)
        ...
        With sAtn
          ...
          .LeaveApplID = ""
        End With
        sAtn = SIS.ATN.atnSiteAttendPosting.UpdateData(sAtn)
      Catch ex As Exception
        sAtn.PostingRemarks = ex.Message
      End Try
      Return sAtn
    End Function
